...
describe("fiProgress", () => {
  ...
  test("the payout is the rate on net worth", () => {
    const fi = fiProgress(500000, 3500);
    assert.equal(fi.yearly, 17500);
    assert.equal(fi.monthly, 1458.33);
  });
  ...
  test("the shortfall is what a month still needs", () => {
    const fi = fiProgress(500000, 4736);
...
describe("netWorthByClass", () => {
  ...
  test("US and international equity are one band", () => {
    ...
    const series = netWorthByClass(
      [chequing],
      [holding("VOO", "US Equity"), holding("DVFD.TO", "Intl Equity")],
      {},
      months,
      { "2026-07": { VOO: 100, "DVFD.TO": 250 } },
    );
...
describe("simpleReturn", () => {
  ...
  test("sale proceeds are netted off what was bought", () => {
    /*
     * INVENTED: 1,000 went in and 300 came back out, so 700 is at risk.
     * Dividing by the gross figure was the bug: it read a real portfolio as
     * gross purchases as the amount invested when much of that was the same
     * money reinvested.
     */
