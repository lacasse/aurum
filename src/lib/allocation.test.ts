...
const positions = [
  ...
  { ticker: "DVFD.TO", name: "Global Equity", marketValue: 3000 },
  { ticker: "SOL", name: "Solana", marketValue: 1000 },
];

describe("drift", () => {
  test("says the gap in points and in money", () => {
    // 60% held against a 20% target, on a [figure redacted] portfolio.
    ...
    assert.equal(btc?.driftValue, 4000, "[figure redacted] more than intended");
  });

  test("under target is a negative drift, not an absent one", () => {
    const { rows } = drift(positions, { "DVFD.TO": 70 });
    const cage = rows.find((r) => r.ticker === "DVFD.TO");
  ...
  test("a target of zero means hold none, which is not the same as no target", () => {
    const { rows, untargeted } = drift(positions, { BTC: 0, "DVFD.TO": 100 });
    const btc = rows.find((r) => r.ticker === "BTC");
  ...
  test("the overweight total is what rebalancing would have to sell", () => {
    const { overweight } = drift(positions, { BTC: 20, "DVFD.TO": 70, SOL: 10 });
