...
/**
 * Keep the last `n` months, or the year to date.
 *
 * Both charts cut here are levels — a value, a compounded return — so a window
 * of n months needs n+1 month-ends: the close it opened on, and one per month
 * after it. Slicing n points covered n-1 months of movement, so "3M" drew two
 * months and said so in the label beside it. The year to date takes the same
 * shape, opening on last December.
 */
    ...
    <Shell
      ...
      action={
        <div className="flex items-center gap-2">
          ...
          {/*
            * Asks before it fetches, because this is the forced refresh: the
            * automatic one already runs on a timer and needs no button. A
            * button that quietly re-ran the same throttled refresh was the one
            * people pressed when prices looked wrong, and it was the one thing
            * that could not help them — everything it asked for was cached,
            * before the close, or out of allowance.
            */}
          <Button
            variant="ghost"
            size="icon"
