...
- **Prod**: a Raspberry Pi (`linux/aarch64`) at `a-local-address`. Runs the live stack.
...
scp /tmp/finance-app-arm64.tar.gz alex@a-local-address:~/finance-app-arm64.tar.gz
...
scp /tmp/aurum-migrate-*.sql.gz alex@a-local-address:~/finance/restore.sql.gz
